...
const DIFFICULTY_BY_NUMBER = { 1: 2.5, 2: 4, 3: 5.5, 4: 6.5, 5: 7.5, 6: 8.5 };
...
const TECHNIQUE_RULES = [
  ...
  [/recurrence|recursive/i, "Recurrence relation"],
  ...
  [/modulo|congruen|divisib|relatively prime|prime number|greatest common divisor/i, "Modular arithmetic and divisibility"],
  ...
  [/continuous|differentiable|derivative|integral|limit/i, "Calculus and estimation"],
...
function parseId(problemName) {
  ...
  return match ? { year: Number(match[1]), session: match[2].toUpperCase(), number: Number(match[3]) } : null;
}

function formatTopic(tag) {
  return TAG_LABELS[tag] || String(tag).replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
}

function chooseArea(tags) {
  const areas = unique(tags.map((tag) => TAG_AREAS[tag]).filter(Boolean));
  return areas.length === 1 ? areas[0] : "Mixed";
}
...
function inferMetadata(problem, parsed, sourceTags) {
  ...
  const techniques = unique(TECHNIQUE_RULES.filter(([pattern]) => pattern.test(evidence)).map(([, label]) => label));
  const concepts = unique(CONCEPT_RULES.filter(([pattern]) => pattern.test(statement)).map(([, label]) => label));
...
function createRecord(problem) {
  ...
  const metadata = inferMetadata(problem, parsed, sourceTags);
  const difficulty = DIFFICULTY_BY_NUMBER[parsed.number];
  return {
...
function createPlaceholder(year, session, number) {
  const difficulty = DIFFICULTY_BY_NUMBER[number];
  return {
...
export function buildPutnamCatalog(rawProblems) {
  const recordsById = new Map();
  for (const problem of Array.isArray(rawProblems) ? rawProblems : []) {
  ...
  const records = [...recordsById.values()].sort((left, right) =>
    right.year - left.year || left.session.localeCompare(right.session) || left.number - right.number,
  );
